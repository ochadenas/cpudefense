    ...
    Node(network, gridX.toFloat(), gridY.toFloat())
{
    ...
    private fun displayChip(canvas: Canvas, rectOnScreen: Rect)
    {
        if (bitmap == null)
            recreateChipBitmap()
        bitmap?.let {
            canvas.withTranslation(rectOnScreen.left.toFloat(), rectOnScreen.top.toFloat()) {
                drawBitmap(it, 0f, 0f, paintBitmap)
            }
    ...
    private fun shootAt(attacker: Attacker)
    {
        ...
        network.commonView.logger()?.debug("Chip %d (%s) shooting at %s (value %s)".format(data.ident, chipDescription(), attacker.toString(), attacker.numberAsString()))
        attacker.logState()
        when (chipData.type)
        {
    ...
    private fun createBitmap(text: String): Bitmap?
    {
        val bitmap: Bitmap? = actualRect?.let {
            ...
            paint.textSize = (CommonView.chipTextSize * network.commonView.textScaleFactor * network.commonView.viewport.userScale) *
                    if (theNetwork.commonView.settings().configUseLargeButtons) 1.0f else 0.96f // multiple sizes possible
            paint.alpha = 255
            paint.typeface = theNetwork.commonView.boldTypeface
    ...
    /** migration function, called for introducing the "refund" property in 1.54 */
    {
        ...
        if (chipData.autoGenerated)
            chipData.refund = chipData.upgradeLevel * GameMechanicolcs.valueOfAutogeneratedChip
        else
            chipData.refund = chipData.value
    }
    ...
    /** some chips have a 'register' where one or several attackers' values can be held. */
    {
        ...
        {
            network.commonView.logger()?.log("Storing attacker %s (value %s) in %s".format(attacker.toString(), attacker.numberAsString(), chipDescription()))
            attacker.logState()
            ...
            theNetwork.commonView.effects?.fade(attacker)
            recreateChipBitmap()
        }
         ...
         */
        {
            try {
                ...
                recreateChipBitmap()
                network.commonView.logger()?.log("Retrieving attacker %s (value %s) from %s".format(attacker.toString(), attacker.numberAsString(), chipDescription()))
                attacker.logState()
                return attacker
            }
            catch (_: ArrayIndexOutOfBoundsException) // register is empty
            {
                return null
            }
        }
