 ...
 */
{
    companion object {
        ...
        fun createStage(stage: Stage, level: Stage.Identifier) {
            val logger = stage.stageView.logger()
            when (level.series) {
                ...
                GameMechanics.SERIES_ENDLESS -> {
                    ...
                    val structure: HashMap<Int, Stage.Data> =
                        Persistency(stage.stageView.theActivity).loadLevelStructure(GameMechanics.SERIES_ENDLESS)
                    val targetDifficulty = 4 + 1.5 * sqrt(level.number.toDouble())
                    if (stage.stageView.settings().keepLevels)
                        structure[level.number]?.let {
                            Stage.fillEmptyStageWithData(stage, it)
                            EndlessStageCreator(stage).createWaves()
                            // remove all auto-generated chips and replace them with new ones
                            stage.chips.values.forEach { chip -> if (chip.chipData.autoGenerated) chip.resetToEmptyChip() }
                            stage.calculateDifficulty()
                        }
                    if (!stage.stageView.settings().keepLevels || !stage.isValidStage() )
                    // in the opposite case, or if no stage data could be loaded
                    // generate a completely new stage layout
                    {
                    ...
                    Persistency(stage.stageView.theActivity).saveLevelStructure(GameMechanics.SERIES_ENDLESS, structure)
            ...
            while (stageDifficulty < targetDifficulty) {
                ...
                if (possibleSlotsForObstacles.isNotEmpty()) {
                    ...
                    when (obstacleSlot.chipData.type) {
                        ...
                        Chip.ChipType.EMPTY -> {
                            obstacleSlot.setType(Chip.obstacleTypes.random())
                            obstacleSlot.chipData.autoGenerated = true
                            obstacleSlot.chipData.refund = GameMechanics.valueOfAutogeneratedChip
                        }
                        else -> {}
                    }
         ...
         */
        {
            ...
            while (additionalCount > 0) {
                ...
                val possibleSlotsForChips = stage.chips.values.filter {
                    it.chipData.type in possibleChipTypesWhereChipsCanBePut && it.chipData.upgradeLevel < GameMechanics.maxLevelOfAdditionalChips}
                if (possibleSlotsForChips.isNotEmpty()) {
                    ...
                    when (slot.chipData.type) {
                        in listOf(Chip.ChipType.SUB, Chip.ChipType.SHR, Chip.ChipType.RES) -> {
                            logger?.log("Chip %s already present, buying an upgrade.".format(slot.chipData.type))
                        ...
                        Chip.ChipType.MEM -> { additionalCount -= 0.2 }  // avoid infinite loop
                        Chip.ChipType.ACC -> { additionalCount -= 0.2 }
                        Chip.ChipType.EMPTY -> {
                            Chip.desiredTypes.random().let {
                                slot.chipData.autoGenerated = true
                                slot.setType(it)
                                slot.chipData.refund = GameMechanics.valueOfAutogeneratedChip  // overwrite the standard value of the chip
                                additionalCount -= Chip.chipStrength[it] ?: 0.0
                                logger?.log("Placing chip %s.".format(slot.chipData.type))
                            }
                        }
                        else -> {}
                    }
                } else {
                    logger?.log("No more chips can be placed")
                    return
                }
            }
